...
public class VerifyLockUnlockInterruptionDuringDownloadFunctionalityTest extends BaseTestV2 {

	String testName = "VerifyLockUnlockInterruptionDuringDownloadFunctionalityTest";
	@Test(dataProvider = "getData")
	public void loginTest(Hashtable<String, String> data) throws Exception{
	if(GlobalVariables.break_Flag)
		throw new SkipException("Skipping the test as it reaches to Home page");
	test = rep.startTest("Verify Lock/Unlock Interruption During Episode Download Functionality Test");
	test.log(LogStatus.INFO, "Starting the test for Lock/Unlock Interruption During Episode Download Functionality - "+VootConstants.DEVICE_NAME);
	// Check run mode
	...
	int lockrow=xls.getRowCount("Smoke_Results")+1;
	xls.setCellData("Smoke_Results", "Testcase Name",lockrow, "Verify if downloading content is resumed post Locking and unlocking the device");	
	    ...
	    if(Utilities.explicitWaitClickable(driver, showspagev2.downloadEpisodesScreenTitle, 20))
		 {
	    	...
	    	test.log(LogStatus.INFO, "Clicking on Download Icon for all the episodes of show - ");
	    	  ...
	    	  test.log(LogStatus.INFO, "Locking the device when download is in progress");
	    	  driver.lockDevice();
	    	  Thread.sleep(1000);
	    	  test.log(LogStatus.INFO, "Unlocking the device and  verifying whether downloads is in Progress");  
	    	  driver.unlockDevice();
	    	 if(Utilities.explicitWaitVisible(driver, showspagev2.downloadIconInDownloadsScreen, 2))
	    	  {
	    		  BasePageV2.reportFail("Download cancelled after unlocking the device");
	    	  }
			    ...
			    if(Utilities.explicitWaitClickable(driver, downloadspagev2.editDownloadsScreen, 5))
				{
			    	...
			    	for(int i=0;i<downloadTitles.size();i++)
			    	{
			    	 ...
			    	 test.log(LogStatus.INFO, "Locking the device when download is in Progress");
			    	  driver.lockDevice();
			    	  Thread.sleep(1000);
			    	  test.log(LogStatus.INFO, "Unlocking the device and verifying whether download is in Progress");
			    	  driver.unlockDevice();
			    		for(int i=0;i<downloadTitles.size();i++)
				    	{
				    	        if(!titles.contains(downloadTitles.get(i)))
