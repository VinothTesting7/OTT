...
public class VerifySwitchAppInterruptionDuringEpisodeDownloadFunctionality extends BaseTestV2 {

	String testName = "VerifySwitchAppInterruptionDuringEpisodeDownloadFunctionality";
	@Test(dataProvider = "getData")
	public void loginTest(Hashtable<String, String> data) throws Exception{
	if(GlobalVariables.break_Flag)
	throw new SkipException("Skipping the test as it reaches to Home page");
	test = rep.startTest("Verify Switch App Interruption during Episode Downloads in Progress Functionality");
	test.log(LogStatus.INFO, "Starting the test for Switch App Interruptions during Episode Downloads in Progress Functionality - "+VootConstants.DEVICE_NAME);
	// Check run mode
	...
	int switchrow=xls.getRowCount("Smoke_Results")+1;
	xls.setCellData("Smoke_Results", "Testcase Name",switchrow, "Verify if downloading content is not interrupted post switching to different app");	
	    ...
	    if(Utilities.explicitWaitClickable(driver, showspagev2.downloadEpisodesScreenTitle, 20))
		 {
	    	...
	    	test.log(LogStatus.INFO, "Clicking on Download Icon for all the episodes of show - ");
	    	 ...
	    	 if(Utilities.explicitWaitVisible(driver, showspagev2.downloadIconInDownloadsScreen, 2))
	    	  {
	    		  BasePageV2.reportFail("Download cancelled after switching to other app");
	    	  }
	    	 else
	    		 BasePageV2.reportPass("Testcase : 'Verify if downloading content is not interrupted post switching to different app' is Passed");
