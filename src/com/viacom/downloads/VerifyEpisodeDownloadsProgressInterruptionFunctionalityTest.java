...
public class VerifyEpisodeDownloadsProgressInterruptionFunctionalityTest extends BaseTestV2 {

	String testName = "VerifyEpisodeDownloadsProgressInterruptionFunctionalityTest";
	@Test(dataProvider = "getData")
	public void loginTest(Hashtable<String, String> data) throws Exception{
	if(GlobalVariables.break_Flag)
		throw new SkipException("Skipping the test as it reaches to Home page");
	test = rep.startTest("Verify Interruption during Episode Downloads in Progress Functionality");
	test.log(LogStatus.INFO, "Starting the test for Interruptions during Episode Downloads in Progress Functionality - "+VootConstants.DEVICE_NAME);
	// Check run mode
	...
	int minrow=xls.getRowCount("Smoke_Results")+1;
	xls.setCellData("Smoke_Results", "Testcase Name",minrow, "Verify if the Downloads appear in Download Episodes screen/Downloads tab when user puts the app to the background and again brings back to the foreground:");	
	
	Xls_Reader xls2 = new Xls_Reader(VootConstants.EXCEL_PATHV2);	
	int minmaxrow=xls2.getRowCount("Smoke_Results")+1;
	xls2.setCellData("Smoke_Results", "Testcase Name",minmaxrow, "Verify if downloading content is resumed post Minimising & Resuming the app");	
	    ...
	    if(Utilities.explicitWaitClickable(driver, showspagev2.downloadEpisodesScreenTitle, 20))
		 {
	    	...
	    	test.log(LogStatus.INFO, "Clicking on Download Icon for all the episodes of show - ");
	    	  ...
	    	  test.log(LogStatus.INFO, "Putting App to the background");
	    	  driver.runAppInBackground(Duration.ofSeconds(3));
	    	  test.log(LogStatus.INFO, "Again bring back App to the foreground and verify whether downloads is in Progress");  	  
	    	 if(Utilities.explicitWaitVisible(driver, showspagev2.downloadIconInDownloadsScreen, 2))
	    	  {
	    		  BasePageV2.reportFail("Download cancelled after putting app to foreground");
	    	  }
	    	 else
	    		 test.log(LogStatus.PASS,"Testcase : 'Verify if the Downloads appear in Download Episodes screen/Downloads tab when user puts the app to the background and again brings back to the foreground:' is Passed");
			    	...
			    	for(int i=0;i<downloadTitles.size();i++)
			    	{
			    	 ...
			    	 test.log(LogStatus.INFO, "Putting App to the background");
			    	  ...
			    	  test.log(LogStatus.INFO, "Again bring back App to the foreground and verify whether downloads is in Progress");  	  
				    	...
				    	{
				    	        if(!titles.contains(downloadTitles.get(i)))
				    	        BasePageV2.reportFail("Episode Title - "+downloadTitles.get(i)+" is not found in Edit Downloads Screen after putting minimize/Resume App");
				    	}
			    	  if(Utilities.explicitWaitVisible(driver, showspagev2.downloadIconInDownloadsScreen, 2))
			    	  {
			    		  BasePageV2.reportFail("Download cancelled after putting app to foreground");
				         ...
				         BasePageV2.smokeresults("", minmaxrow, "Pass");
