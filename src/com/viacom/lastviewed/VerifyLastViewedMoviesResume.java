...
public class VerifyLastViewedMoviesResume extends BaseTestV2{
	
	String testName = "VerifyLastViewedMoviesResume";
	@Test(dataProvider = "getData")
	public void VerifyLastViewedMoviesResume(Hashtable<String, String> data) throws Exception 
	{		
		test = rep.startTest("VerifyLastViewedMoviesResume");
		...
		// P1 VK_2071 Verify the playback status for partially watched Movie content in Last viewed tray (LV in watch tab)
		// P1 VK_2072 Verify the playback status for Completetly watched Movie content in Last viewed tray (LV in watch tab)
		// P1 VK_2073 Verify Last viewed Movie cards are removed from Watch Tab - Last view tray by tapping on 'Clear' link text (LV in watch tab)
		...
		if(foundMoviesTrayFromAPI==true) {
						...
						if(Utilities.explicitWaitVisibleNew(driver, kidsplayerv2.playerCurrentDuration, 10)) {
							try {
								 ...
								 if(completelyWatchedTime<=30) {
									 ...
									 if(!Utilities.setResultsKids("VK_2072", "Pass")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");
								 }
								 else {
									 ...
									 if(!Utilities.setResultsKids("VK_2072", "Fail")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");
								 }
							}
							catch(Exception e) {
								test.log(LogStatus.FAIL, "Player current duration is not displayed"); 
		    ...
		    if(Utilities.explicitWaitVisibleNew(driver, homepagev2.lvMovieSecondItemTitle, 10)) {
		    	...
		    	if(lastViewed2.contains(movieTitle2)) {
						...
						if(Utilities.explicitWaitVisibleNew(driver, kidsplayerv2.playerCurrentDuration, 10)) {
							try {
								 ...
								 if(partiallyWatchedSecondTime>partiallyWatchedFirstTime) {
									 ...
									 if(!Utilities.setResultsKids("VK_2071", "Pass")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");
								 }
								 else {
									 ...
									 if(!Utilities.setResultsKids("VK_2071", "Fail")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");
								 }
							}
							catch(Exception e) {
								test.log(LogStatus.FAIL, "Player current duration is not displayed"); 
		    		...
		    		test.log(LogStatus.FAIL, lastViewed2+" is the second latest movie under last viewed in Watch tab instead of "+movieTitle2);
		    	}
		    }
		    else {
				...
				try {
					...
					try {
						...
						if(Utilities.explicitWaitVisibleNew(driver, homepagev2.watchTabLVClear, 10)) {
							...
							test.log(LogStatus.FAIL, "Verify Last viewed Movie cards are removed from Watch Tab - Last view tray by tapping on 'Clear' link text is Fail");	
							if(!Utilities.setResultsKids("VK_2073", "Fail")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");
							BasePageV2.takeScreenshot();
						}
						else {
							test.log(LogStatus.INFO, "Clear button is not displayed, tray has been cleared");
							test.log(LogStatus.PASS, "Verify Last viewed Movie cards are removed from Watch Tab - Last view tray by tapping on 'Clear' link text is Pass");	
							if(!Utilities.setResultsKids("VK_2073", "Pass")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");
						}	
					}
					catch(Exception e) {
