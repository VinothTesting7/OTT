...
//VK_2311 - Verify the functionality when user taps on Favorite icon from book programme info for the 1st time:
//VK_2313 - Verify the functionality when user taps on Favorite icon from ebook programme info from 2nd time onwards:

public class VerifyFavoriteIconFunctionalityInEbookInfoPageTest extends BaseTestV2 {
	String testName = "VerifyFavoriteIconFunctionalityInEbookInfoPageTest";
	@Test(dataProvider = "getData")
	public void loginTest(Hashtable<String, String> data) throws Exception{
	if(GlobalVariables.break_Flag)
		throw new SkipException("Skipping the test as it reaches to Home page");
	test = rep.startTest("VerifyFavoriteIconFunctionalityInEbookInfoPageTest");
	test.log(LogStatus.INFO, "Starting the test for Verifying the ebook program Info page functionality: "+VootConstants.DEVICE_NAME);
	// Check run mode
	...
	test.log(LogStatus.INFO, "Verifying the Favorite icon functionality of book progr");	  
	...
	if(Utilities.explicitWaitClickableNew(driver,readpagev2.favIconBookDetails, 30))
			           ...
			           if(Utilities.explicitWaitVisibleNew(driver, showspagev2.favIconPopup, 30))
						{
							...
							if(Utilities.explicitWaitClickableNew(driver, showspagev2.favIconPopupOkButton, 30))
							{
								...
								Thread.sleep(3000);
								test.log(LogStatus.PASS, "Testcase : 'Verify the functionality when user taps on Favorite icon from book programme info for the 1st time:' is Passed");
								  if(!Utilities.setResultsKids("VK_2311", "Pass")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");
						...
						{
							  if(!Utilities.setResultsKids("VK_2311", "Fail")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");	
							test.log(LogStatus.FAIL, "Pop up is not displayed after favoriting book content in ebook program info for first time");
						}
	  ...
	  Thread.sleep(500);
	              ...
	              if(Utilities.explicitWaitClickable(driver, watchpagev2.firstItemInCarousal, 3)){
	            	  ...
	            	  BasePageV2.reportFail("Not able to tap on masthead carousal ebook content");
							if(Utilities.explicitWaitClickableNew(driver, readpagev2.favIconBookDetails, 30))
							{
								...
								if(!Utilities.explicitWaitVisibleNew(driver, showspagev2.favIconPopup, 5))
								{
										...
										test.log(LogStatus.PASS, "Testcase : 'Verify the functionality when user taps on Favorite icon from ebook programme info from 2nd time onwards:' is Passed");
										if(!Utilities.setResultsKids("VK_2313", "Fail")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");	
								}
								else
								{
									if(!Utilities.setResultsKids("VK_2313", "Fail")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");	
									test.log(LogStatus.FAIL, "Pop up is displayed after favoriting book content in ebook program info page for second time");
								}
							if(Utilities.explicitWaitVisibleNew(driver, readpagev2.favIconBookDetails, 30))
								{	
								readpagev2.favIconBookDetails.click();
