...
//VK_2318 - Verify the functionality of OK button from Favorite pop up for an episode content:
//VK_2321 - Verify the functionality of OK button from Favorite pop up for an movie content:


public class VerifyFavoritePopUpFunctionalityInPlayerTest extends BaseTestV2 {
	String testName = "VerifyFavoritePopUpFunctionalityInPlayerTest";
	@Test(dataProvider = "getData")
	public void loginTest(Hashtable<String, String> data) throws Exception{
	if(GlobalVariables.break_Flag)
		throw new SkipException("Skipping the test as it reaches to Home page");
	test = rep.startTest("VerifyFavoritePopUpFunctionalityInPlayerTest");
		  ...
		  if(Utilities.explicitWaitVisible(driver,playerpagev2.videoPlayer, 3)) {
				  ...
				  test.log(LogStatus.INFO, "Verifying the Favorite icon functionality of VOD Player");	  
					...
					if(Utilities.explicitWaitClickableNew(driver, playerpagev2.playerFavoriteButton, 30))
					{
						...
						if(Utilities.explicitWaitVisibleNew(driver, showspagev2.favIconPopup, 30))
						{
							...
							if(Utilities.explicitWaitClickableNew(driver, showspagev2.favIconPopupOkButton, 30))
							{
								...
								Thread.sleep(3000);
								  ...
								  if(!Utilities.setResultsKids("VK_2318", "Pass")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");
									if(Utilities.explicitWaitClickableNew(driver, playerpagev2.playerFavoriteButton, 30))
									{
										test.log(LogStatus.PASS, "Testcase : 'Verify the functionality of OK button from Favorite pop up for an episode content:' is Passed");
										if(!Utilities.setResultsKids("VK_2318", "Pass")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");	
									}
									else
									{
										test.log(LogStatus.FAIL, "Pop up ok button functionality failed");
										if(!Utilities.setResultsKids("VK_2318", "Fail")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");	
									}
							}
							else
							{
						...
						else if(Utilities.explicitWaitClickableNew(driver, playerpagev2.playerFavoriteButton, 30))
						{
							playerpagev2.playerFavoriteButton.click();
							Thread.sleep(3000);
	...
	//Click on the first carousal item
            ...
            test.log(LogStatus.INFO, "Tray Title is -"+trayTitle);
			...
			Utilities.verticalSwipe(driver, trayLoc, "Visible", 20);
			boolean trayPresence=false;
			try{
				driver.findElement(By.xpath("//android.widget.TextView[@text='"+trayTitle+"']"));
				trayPresence=true;
				Utilities.verticalSwipe(driver, trayLoc, "Clickable", 4);
			}
			catch(Exception e){}
			...
			playerpagev2.pauseVideo();
							...
							if(Utilities.explicitWaitClickableNew(driver, showspagev2.favIconPopupOkButton, 30))
							{
								...
								Thread.sleep(3000);
									if(Utilities.explicitWaitClickableNew(driver, playerpagev2.playerFavoriteButton, 30))
									{
										...
										if(!Utilities.setResultsKids("VK_2321", "Pass")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");	
									}
									else
									{
										test.log(LogStatus.FAIL, "Pop up ok button functionality failed");
										if(!Utilities.setResultsKids("VK_2321", "Fail")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");	
									}
							}
							else
							{
				 ...
				 BasePageV2.reportFail("Tray not found for movies");
	 }}
