...
public class MoviePlayernetWorkInterruption_P2 extends BaseTestV2{
	
String testName = "MoviePlayernetWorkInterruption_P2";
	...
	public void MoviePlayernetWorkInterruption_P2(Hashtable<String, String> data) throws Exception{	
	test = rep.startTest("MoviePlayernetWorkInterruption_P2");
	...
	// VK_2101   - Verify Movie playback state when 'Cellular Playback' is in OFF state and app is launched in Cellular network.
		...
		//Hit Watch API and find the name of the Movies tray
				...
				if(foundMoviesTrayFromAPI==true) {
					...
					// Turn Off the Cellular playback in Device Settings
					settingsPage.cellularPlayBackSwichOFF();
					Thread.sleep(200);
				...
				// Click on play button in movie info page
				  ...
				  if(Utilities.explicitWaitVisible(driver, watchpagev2.cellularplaybackOffMsg, 20)) {
					  test.log(LogStatus.PASS, "Verify Movie playback state when 'Cellular Playback' is in OFF state and app is launched in Cellular network.");
						if(!Utilities.setResultsKids("VK_2101", "Pass")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");	
						watchpagev2.okBtn.click();
					}else {

						test.log(LogStatus.FAIL, "Verify Movie playback state when 'Cellular Playback' is in OFF state and app is launched in Cellular network.");
						if(!Utilities.setResultsKids("VK_2101", "Fail")) test.log(LogStatus.WARNING, "TC ID not found in the tc document");	
						watchpagev2.okBtn.click();
					}
				  ...
				  // Turn ON the Cellular Switch to ON state
				  ...
				  settingsPage.cellularPlayBackSwichON();
